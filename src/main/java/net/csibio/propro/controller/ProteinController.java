...
public class ProteinController {
  ...
  @Autowired ProteinService proteinService;
  ...
  Result add(
      @RequestParam(value = "createTag", required = false) String createTag,
      ...
      @RequestParam(value = "maxPepLen", required = false) int maxPepLen)
      throws IOException {
    ...
    Result<List<ProteinDO>> result =
        proteinService.importFromFasta(
            inputStream, libFile.getOriginalFilename(), reviewed, minPepLen, maxPepLen);
    if (result.isFailed()) {
      return result;
    }
    if (createLibrary) {
      ...
      proteinService.proteinToPeptide(
          libraryDO.getId(), data, minPepLen, maxPepLen, spModel, isotope);
    }
    return Result.OK();
  }
}
