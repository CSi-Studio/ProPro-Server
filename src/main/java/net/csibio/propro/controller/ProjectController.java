...
public class ProjectController {
            ...
            result
                    ...
                    .forEach(
                            projectVO -> {
                                ...
                                projectVO.setRunCount(
                                        runService.count(new RunQuery().setProjectId(projectVO.getId())));
                                projectVO.setOverviewCount(
                                        overviewService.count(new OverviewQuery().setProjectId(projectVO.getId())));
                            });
        }
        return result;
    }
    ...
    Result<ProjectBeforeAddVO> beforeAdd() {
        ...
        vo.setInsLibs(
                allLibs.stream()
                        .filter(
                                idNameType ->
                                        idNameType.type() != null
                                                && idNameType.type().equals(LibraryType.INS.getName()))
        ...
        vo.setAnaLibs(
                allLibs.stream()
                        .filter(
                                idNameType ->
                                        idNameType.type() != null
                                                && idNameType.type().equals(LibraryType.ANA.getName()))
                        .collect(Collectors.toList()));
    ...
    Result removeAnalyse(@RequestParam("projectId") String projectId) {
        ...
        List<IdName> idNameList =
                overviewService.getAll(new OverviewQuery().setProjectId(project.getId()), IdName.class);
