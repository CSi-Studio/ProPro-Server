...
public class PeptideController extends XController<LibraryDO, LibraryQuery, LibraryService> {
  ...
  Result getPeptideRefs(
      @RequestParam(value = "libraryId") String libraryId,
      @RequestParam(value = "protein") String protein) {
    List<PeptideV1> peptideList =
        peptideService.getAll(new PeptideQuery(libraryId).setProtein(protein), PeptideV1.class);
    return Result.OK(peptideList);
  }

  @GetMapping(value = "/predict")
  Result predict(
      @RequestParam(value = "peptideId") String peptideId,
    ...
    List<FragmentInfo> fragmentInfos =
        simulateService.predictFragment(peptide.getSequence(), spModel, iso, limit);
    return Result.OK(fragmentInfos);
  }

  @RequestMapping(value = "/updateFragment", method = RequestMethod.POST)
  Result<PeptideDO> updateFragment(
      @RequestParam(value = "peptideId") String peptideId, @RequestBody FragmentInfo[] fragments) {
    PeptideDO peptide = peptideService.getById(peptideId);
    if (peptide == null) {
      return Result.Error(ResultCode.OBJECT_NOT_EXISTED);
    }
