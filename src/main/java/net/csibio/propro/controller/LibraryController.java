...
public class LibraryController extends XController<LibraryDO, LibraryQuery, LibraryService> {
    ...
    Result add(LibraryUpdateVO libraryUpdateVO) {
        ...
        library.setDescription(libraryUpdateVO.getDescription());
        library.setType(libraryUpdateVO.getType());
        library.setOrganism(libraryUpdateVO.getOrganism() != null ? Arrays.stream(libraryUpdateVO.getOrganism().toLowerCase().split(SymbolConst.COMMA)).filter(s -> !s.isEmpty()).collect(Collectors.toSet()) : null);
    ...
    Result clone(@RequestParam(value = "id") String sourceLibId,
    ...
    Result generateDecoys(@RequestParam(value = "libraryId") String libraryId,
