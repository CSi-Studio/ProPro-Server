...
public class AnalyzeController {
  ...
  Result<PrepareAnalyzeVO> prepare(@RequestParam("projectId") String projectId) {
    ...
    List<IdName> insLibList =
        libraryService.getAll(new LibraryQuery().setType(LibraryType.INS.getName()), IdName.class);
    List<IdName> anaLibList =
        libraryService.getAll(new LibraryQuery().setType(LibraryType.ANA.getName()), IdName.class);
  ...
  Result analyze(
    ...
    List<RunDO> runList =
        runService.getAll(new RunQuery().setProjectId(projectId).setIds(runIdList));
    ...
    if (onlyIrt) {
      ...
      for (RunDO run : runList) {
        TaskDO task =
            new TaskDO(TaskTemplate.EXTRACT_PEAKPICK_SCORE, "Analyze-EPPS-" + project.getName());
        taskService.insert(task);
        AnalyzeParams params = new AnalyzeParams(method);
