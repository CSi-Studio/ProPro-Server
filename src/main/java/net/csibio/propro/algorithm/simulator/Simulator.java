...
public class Simulator extends PeptideSpectrumPair {
	...
	public void addyPeaks() {
		...
		for (int i = skel.length() - 1; i > 0; i--) {
			...
			if (i == 1) {
				...
				float exP = staticValue.lAAProbs[staticValue.getIndex(n0c)]
						* staticValue.rAAProbs[staticValue.getIndex(n1c)];
				if (len < staticValue.posProbs.length) {
					exP *= staticValue.posProbs[len];
				}
	...
	public Float getLnRatio(int pos) {
		...
		Instance instance = staticValue.getInstance(this.peptide, pos, 0);
		
		double ratio= staticValue.parameter.getlnR(instance);
		/*
		if(ratio==Double.NaN)
		{
			return null;
