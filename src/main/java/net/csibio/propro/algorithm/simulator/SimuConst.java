...
public class SimuConst {
    ...
    static float[][] getAA(Peptide pep, int fragPos) {
        float[][] aaA = new float[SimuConst.AAend - SimuConst.AAbegin + 1][20];
        char[] aa = pep.seqAA.toCharArray();
        for (int i = SimuConst.AAbegin; i <= SimuConst.AAend; i++) {
            int nn = fragPos + i;
            if (nn >= 0 && nn < aa.length - 1) {
                char a = pep.seqAA.charAt(nn);
                int index = SimuConst.mapAAIndex[Character.getNumericValue(a)];
                aaA[i - SimuConst.AAbegin][index] = 1;
            }
        }
    ...
    static float[][] getKR(Peptide pep, int fragPos) {
        float[][] krA = new float[2][SimuConst.KRend - SimuConst.KRbegin
                + 1];
        char[] aa = pep.seqAA.toCharArray();
        for (int i = SimuConst.KRbegin; i <= SimuConst.KRend; i++) {
            ...
            if (nn >= 0 && nn < aa.length) {
                char a = pep.seqAA.charAt(nn);
                if (a == 'K') {
                    krA[0][i - SimuConst.KRbegin] = 1;
                }
                if (a == 'R') {
                    krA[1][i - SimuConst.KRbegin] = 1;
                }
    ...
    static float[] getCproton(Peptide pep, int fragPos) {
         ...
         * ABCKD*EFGR
         */
        float[] cA = new float[SimuConst.cLen + 1];
        int proton = pep.seqAA.length();
        if (proton - fragPos <= SimuConst.cLen)
    ...
    static float[] getNterm(Peptide pep, int fragPos) {
         ...
         * ABCKD*EFGR
         */
        float[] nA = new float[SimuConst.nLen + 1];
        // if (fragPos <= staticValue.nLen)
        // nA[fragPos]=1;
        int ndis = (int) (SimuConst.nLen * fragPos / pep.seqAA.length());
        nA[ndis] = 1;
    ...
    static float[][] getDikeA(Peptide pep, int fragPos) {
        float[][] dikeA = new float[SimuConst.dickLen][20];
        if (fragPos >= 4)
            return dikeA;
        if (fragPos - 1 < SimuConst.dickLen) {
            char ch = pep.seqAA.charAt(fragPos - 1);
            dikeA[fragPos - 1][SimuConst.getIndex(ch)] = 1;
        }

        if (fragPos < SimuConst.dickLen) {
            char ch = pep.seqAA.charAt(fragPos);
            dikeA[fragPos][SimuConst.getIndex(ch)] = 1;
        }
    ...
    static ArrayList<String> getNames() {
        ...
        for (int p = SimuConst.AAbegin; p <= SimuConst.AAend; p++) {
        ...
        for (int c = 1; c <= SimuConst.cLen; c++) {
            names.add("Cproton_X_" + c);
        }
        for (int c = 1; c <= SimuConst.nLen; c++) {
            names.add("Ndis_X_" + c);
        }
        for (int j = 0; j < SimuConst.dickLen; j++)
            for (int i = 0; i < SimuConst.aa_array.length; i++) {
                names.add("dR_" + SimuConst.aa_array[i] + "_" + j);
            }
