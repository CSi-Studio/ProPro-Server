...
public class Parameters2 implements Parameter {
	...
	public Parameters2() {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (type.equals("AA")) {
				parameter.AA[pos - staticValue.AAbegin][staticValue
						.getIndex(aa)] = weight;
			}
			if (type.equals("KR")) {
				// System.out.println(pos+"\t"+aa);
				if (aa == 'K')
					parameter.KR[0][pos - staticValue.KRbegin] = weight;
				if (aa == 'R')
					parameter.KR[1][pos - staticValue.KRbegin] = weight;
			}
			...
			if (type.equals("dR")) {
				parameter.dickA[pos][staticValue.getIndex(aa)] = weight;
			}
		}
